...
function usage() {
  console.error('Usage: node scripts/runtime-dom-diff.js <original.html> <refactored.html>');
  process.exit(2);
}
...
(async () => {
  console.log('Capturing runtime DOM for', aPath);
  const aHtml = await captureRenderedHtml(aPath);
  console.log('Capturing runtime DOM for', bPath);
  const bHtml = await captureRenderedHtml(bPath);
  // save snapshots for inspection
  writeFileSync('.runtime_a.html', aHtml, 'utf8');
  writeFileSync('.runtime_b.html', bHtml, 'utf8');
  const { missing, diffs } = diffHtml(aHtml, bHtml);
  console.log('Runtime DOM diff report');
  ...
  if (diffs.length) {
    console.log('Attribute/tag/class/inner differences:');
    diffs.slice(0, 500).forEach(d => console.log(` - ${d.path} :: ${d.type} => original="${d.a}" refactored="${d.b}"`));
  } else {
    console.log('No attribute/class/tag/inner differences detected.');
  }
})();
