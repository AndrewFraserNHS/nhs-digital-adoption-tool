...
describe('utils', () => {
  ...
  it('triggers file download flow', () => {
    ...
    const createUrlSpy = vi.spyOn(URL as unknown as { createObjectURL: (blob: Blob) => string }, 'createObjectURL');
    const revokeSpy = vi.spyOn(URL as unknown as { revokeObjectURL: (url: string) => void }, 'revokeObjectURL');
