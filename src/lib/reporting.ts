...
import { downloadFile, escapeCsv, escapeHtml } from './utils';
...
export function generateActionPlanReport(data: ActionPlanReportData): string {
  ...
  if (rows.length === 0) {
    bodyHTML =
      '<p style="text-align:center;color:#4b5563;">No actions have been defined in this assessment.</p>';
  } else {
    const tableRows = rows
      .map((row, index) => {
...
export function exportActionPlanReportToCSV(
  rows: ActionPlanReportRow[],
  filename = 'action-plan-report.csv'
) {
  ...
  for (const r of rows) {
    ...
    lines.push(
      [r.theme, r.text, r.owner || '', r.status || 'Planned', dueDateFormatted]
        .map(escapeCsv)
        .join(',')
    );
  }
...
export function exportMaturityReportToCSV(
  rows: Array<Record<string, unknown>>,
  filename = 'maturity-report.csv'
) {
  if (!rows || rows.length === 0) {
    return;
  }
