...
describe('Bundled component-actions data quality (Pathway 1)', () => {
  it.each(Object.entries(PATHWAY_1_SOURCES))(
    ...
    (_componentId, source) => {
      const missing = source.actions.filter((action) => !action.category || !action.category.trim());
      const unrecognised = source.actions.filter(
        (action) =>
          action.category && action.category.trim() && !(ACTION_TYPES as readonly string[]).includes(action.category)
      );
